...
main(int argc, char *argv[])
{
  ...
  sb.magic = FSMAGIC;
  sb.size = xint(FSSIZE);
  sb.blocks_num = xint(nblocks);
  sb.inodes_num = xint(NINODES);
  sb.log_num = xint(nlog);
  sb.log_start = xint(2);
  sb.inode_start = xint(2+nlog);
  sb.bmap_start = xint(2+nlog+ninodeblocks);
...
void
winode(uint inum, struct disk_inode *ip)
{
  char buf[BLOCK_SIZE];
  uint bn;
...
void
rinode(uint inum, struct disk_inode *ip)
{
  char buf[BLOCK_SIZE];
  uint bn;
  struct disk_inode *dip;
