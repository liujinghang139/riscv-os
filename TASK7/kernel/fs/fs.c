...
r_superblock(uint dev){
  ...
  memmove(&superblock, b->data, BSIZE);
  relse_buf(b);
}
...
itable_init(){
  ...
  initlock(&itable.lock, "itable_lock");
  for(i = 0;i < NINODE;i++){
...
update_inode(struct inode *inode){
  ...
  buf = read_buf(inode->dev, IBLOCK(inode->inum, superblock));
  
  dinode = (struct disk_inode*)buf->data + (inode->inum % IPB);
  dinode->file_type = inode->type;
  dinode->major = inode->major;
  dinode->minor = inode->minor;
  dinode->links = inode->nlink;
...
lock_inode(struct inode *inode){
  ...
  if(!inode->valid){
    buf = read_buf(inode->dev, IBLOCK(inode->inum, superblock));
    dinode = (struct disk_inode *)buf->data + (inode->inum%IPB);
    inode->type = dinode->file_type;
    inode->major = dinode->major;
    inode->minor = dinode->minor;
    inode->nlink = dinode->links;
...
unlock_inode(struct inode *inode){
  if(inode == 0 || !holding_sleeplock(&inode->lock) || inode->ref == 0){
    panic("unlock_inode\n");
  }
...
putback_inode(struct inode *inode){
  ...
  if(inode->ref == 1 && inode->nlink == 0 && inode->valid){
...
reclaim_inode(int dev){
  for(int inum = 1; inum < superblock.ninodes;inum++){
    struct buffer *buf;
    struct disk_inode* dinode;
    struct inode* ip = 0;
...
read_inode(struct inode *inode, int user_dst, uint64 dst, uint off, uint n){
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    blocknum = bmap(inode, off/BSIZE);
    if(blocknum == 0){
      break;
    }
    buf = read_buf(inode->dev, blocknum);
    m = min((BSIZE - off%BSIZE), (n-tot));
    if(either_copyout(user_dst, dst, buf->data + (off % BSIZE), m) == -1){
      relse_buf(buf);
      return -1;
    }
    relse_buf(buf);
...
int write_inode(struct inode *inode, int user_src, uint64 src, uint off , uint n){
  ...
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    uint blocknum = bmap(inode, off/BSIZE);
    if(blocknum == 0)
      break;
    buf = read_buf(inode->dev, blocknum);
    m = min(n - tot, BSIZE - off%BSIZE);
    if(either_copyin(buf->data + (off % BSIZE), user_src, src, m) == -1){
      relse_buf(buf);
      break;
    }
    log_write(buf);
